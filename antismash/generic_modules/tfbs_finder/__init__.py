...
REQUIRES = [(PWM_PATH, False)]  # False = required


# Required top-level function for plugin system
...
# Required top-level function for generic genome-wide modules
def check_prereqs(options) -> List[str]:
    """Check required input files exist (e.g., PWM JSON)."""
    if not os.path.exists(PWM_PATH):
        return [f"Missing PWM JSON file at: {PWM_PATH}"]
...
def run_analyses(seq_records: List[SeqRecord], options) -> None:
    """Main genome-wide execution function called by run_antismash.py."""
    output.run_analyses(seq_records, options)
...
def will_handle(product: str) -> bool:
    return True  # always show TFBS if available
